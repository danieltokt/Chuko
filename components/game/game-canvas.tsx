...
// Chuko projectile colors - like the colorful team characters
const CHUKO_TEAM = [
  { body: '#D2691E', face: '#8B4513', outfit: '#FF6B6B', name: 'Altyn' },    // Brown/coral
  { body: '#FFB6C1', face: '#DDA0DD', outfit: '#FF69B4', name: 'Gulzat' },   // Pink
  { body: '#4169E1', face: '#1E3A8A', outfit: '#6366F1', name: 'Azamat' },   // Blue
  { body: '#228B22', face: '#006400', outfit: '#22C55E', name: 'Ermek' },    // Green
]

// Level configurations - blocks positioned relative to right side
const LEVELS = [
  ...
  // Level 5 - Complex fortress
  {
    targets: 3,
    chukos: 5,
    blocks: [
      { x: 450, y: 460, w: 25, h: 50, type: 'stone' as const },
      { x: 550, y: 460, w: 25, h: 50, type: 'stone' as const },
      { x: 650, y: 460, w: 25, h: 50, type: 'stone' as const },
      { x: 750, y: 460, w: 25, h: 50, type: 'stone' as const },
      { x: 500, y: 415, w: 130, h: 20, type: 'wood' as const },
      { x: 700, y: 415, w: 130, h: 20, type: 'wood' as const },
      { x: 500, y: 375, w: 35, h: 45, type: 'target' as const },
      { x: 700, y: 375, w: 35, h: 45, type: 'target' as const },
      { x: 600, y: 385, w: 20, h: 80, type: 'wood' as const },
      { x: 600, y: 325, w: 80, h: 20, type: 'wood' as const },
      { x: 600, y: 285, w: 35, h: 45, type: 'target' as const },
    ]
  },
  // Level 6
  {
    ...
    chukos: 6,
    blocks: [
      { x: 480, y: 460, w: 20, h: 50, type: 'stone' as const },
      { x: 580, y: 460, w: 20, h: 50, type: 'stone' as const },
      { x: 680, y: 460, w: 20, h: 50, type: 'stone' as const },
      { x: 530, y: 415, w: 120, h: 20, type: 'wood' as const },
      { x: 630, y: 415, w: 120, h: 20, type: 'wood' as const },
      { x: 500, y: 375, w: 35, h: 45, type: 'target' as const },
      { x: 580, y: 375, w: 35, h: 45, type: 'target' as const },
      { x: 660, y: 375, w: 35, h: 45, type: 'target' as const },
      { x: 580, y: 330, w: 20, h: 60, type: 'wood' as const },
      { x: 580, y: 275, w: 35, h: 45, type: 'target' as const },
    ]
  },
]
...
export default function GameCanvas({ level, onBack, onComplete }: GameCanvasProps) {
  ...
  // Draw white/bone chuko projectile with character features
  ...
  // Draw white/bone target chuko (enemy)
  const drawTargetChuko = useCallback((
    ...
    // Body gradient - white/cream bone color like the reference image
    ...
    // Bone/chuko shape - like the reference image
    ctx.beginPath()
    // Top bumps
    ...
    // Eyes - small dots like the reference
  ...
  const drawTrajectory = useCallback((
    ...
    // Step simulation - each step = 3 Matter.js frames for longer preview
    // Matter.js gravity adds ~0.0167 per frame, frictionAir=0.01 reduces velocity
    const steps = 60
    ...
    for (let i = 0; i < steps; i++) {
      for (let f = 0; f < framesPerStep; f++) {
        vy += 0.0167       // gravity per frame
        vx *= (1 - 0.01)   // frictionAir
        vy *= (1 - 0.01)   // frictionAir
        x += vx
        y += vy
      }
  ...
  // Draw slingshot - positioned on ground
  const drawSlingshot = useCallback((ctx: CanvasRenderingContext2D, x: number, y: number, groundY: number) => {
    ...
    // Fork tops (rubber attachment points)
  ...
  const drawYurt = useCallback((ctx: CanvasRenderingContext2D, x: number, y: number, scale: number = 1) => {
    ...
    // Decorative band (red with pattern)
    ctx.fillStyle = '#B22222'
    ctx.fillRect(-78, 5, 156, 18)
    
    // Kyrgyz pattern on band
    ...
    // Lattice pattern on walls (kerege)
  ...
  const initGame = useCallback(() => {
    ...
    // Ground - positioned at visual ground level
    ...
    levelConfigRef.current.blocks.forEach(block => {
      ...
      if (block.type === 'target') {
        body = Matter.Bodies.rectangle(block.x, block.y, block.w, block.h, {
          label: 'target',
          density: 0.002,
          friction: 0.8,
          restitution: 0.2,
        ...
        body = Matter.Bodies.rectangle(block.x, block.y, block.w, block.h, {
          label: block.type,
          density: isStone ? 0.006 : 0.003,
          friction: 0.8,
          restitution: isStone ? 0.1 : 0.3,
        })
      }
    ...
    // Create first bird (static until launched)
    ...
    Matter.Events.on(engine, 'collisionStart', (event) => {
      event.pairs.forEach(pair => {
        ...
        // Block destruction from high speed impact
    ...
    const render = () => {
      ...
      // Slingshot position and ground
      const slX = slingshotPos.x
      const groundY = height - 70
      const forkY = groundY - 110 // Fork height
      const birdRestY = height - 180 // Where bird sits
      
      // Back elastic (behind bird)
      ...
      bodies.forEach(body => {
        ...
        if (body.label === 'bird') {
          // Only draw if not the current bird being aimed
      ...
      if (currentBirdRef.current && (isDraggingRef.current || gameStateRef.current === 'aiming')) {
        ...
        // Bird waiting in slingshot
        const colorIndex = birdsUsedRef.current % CHUKO_TEAM.length
        drawChukoProjectile(ctx, slX, birdRestY, 22, colorIndex)
      }

      // Waiting birds queue (left side on ground)
    ...
    // Mouse/touch handling - SMOOTH DIRECT POSITIONING
    ...
    const onMove = (e: MouseEvent | TouchEvent) => {
      ...
      // Clamp to max pull distance and only allow pulling back (left)
    ...
    const onEnd = () => {
      ...
      // Remove static bird and create a new dynamic one at the pulled position
      ...
      // Power is proportional to how far pulled (max pull = maxPullDistance = 120px)
        ...
        <div className="absolute bottom-24 left-1/2 -translate-x-1/2 z-10 text-center">
          <div className="bg-black/60 px-6 py-3 rounded-xl backdrop-blur-sm">
            <p className="text-white text-lg font-medium" style={{ fontFamily: "var(--font-fredoka), 'Fredoka', sans-serif" }}>
              Drag the Chuko back and release to launch!
            </p>
          </div>
        </div>
      )}
          ...
          >
            ...
            <div className="flex gap-4 justify-center">
              ...
              {gameState === 'won' && (
                ...
                >
                  <Play className="w-6 h-6" /> Continue
                </button>
              )}
            </div>
          </div>
